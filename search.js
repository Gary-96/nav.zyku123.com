    ...
    engineItems.forEach(item => {
        item.addEventListener('click', function() {
            ...
            if (currentEngine === 'local') {
                if (keyword) {
                    console.log('触发站内搜索:', keyword);
                }
                return;
            }

            if (config && config.action && config.action !== '#') {
                if (keyword) {
                    let queryParam = 'q';
                    if (currentEngine === 'baidu') queryParam = 'wd';
                    
                    const searchUrl = `${config.action}?${queryParam}=${encodeURIComponent(keyword)}`;
                    window.open(searchUrl, '_blank');
                } else {
                    // 如果空值，提取首页跳转
                    try {
                        const urlObj = new URL(config.action);
