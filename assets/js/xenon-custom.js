...
function setup_sidebar_menu()
{
	if(public_vars.$sidebarMenu.length)
	{
		...
		$items_with_subs.each(function(i, el)
		{
			...
			{
				// 如果侧边栏折叠且不是折叠/展开按钮，先展开侧边栏
				if(public_vars.$sidebarMenu.hasClass('collapsed') && !$a.hasClass('sidebar-toggle'))
				{
					ev.preventDefault();
					// 展开侧边栏
					public_vars.$sidebarMenu.removeClass('collapsed');
		...
		// 处理没有子菜单的菜单项点击
		public_vars.$sidebarMenu.find('.main-menu > li:not(.has-sub) > a').on('click', function(ev)
		{
			// 如果侧边栏折叠，先展开侧边栏
		...
		// 为所有菜单项添加 tooltip 属性
		public_vars.$sidebarMenu.find('.main-menu > li > a').each(function()
		{
			var $a = $(this);
			var text = $a.find('span').text().trim();
			if(text && !$a.attr('data-tooltip'))
			{
				$a.attr('data-tooltip', text);
			}
		});
	}
}
